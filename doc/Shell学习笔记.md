...
^匹配一行的开头
  ```
  cat /etc/passwd | grep ^a   #查看以a为开头的行
  ```
- 特殊字符：**$**
$匹配一行的结尾
  ```
  cat /etc/passwd | grep h$   #查看以h为结尾的行
  ```
- 特殊字符：**.**
.匹配一个任意的字符
  ```
  cat etc/passwd | grep r..t  #查看所有包含r..t的行
  ```
- 特殊字符：**\***
\*不单独使用，和上一个字符连用，表示匹配上一个字符0次或多次
  ```
  cat /etc/passwd | grep ro*t  #rt rot root rooot等所在行都会被筛选
  ```
- 字符区间：**[]**
[]表示匹配某个范围内的一个字符，例如
[6,8]------匹配 6 或者 8
...
[a-c,e-f]---匹配 a-c 或者 e-f 之间的任意字符
  ```
   cat /etc/passwd | grep r[a,b,c]*t    #会匹配 rt,rat, rbt, rabt, rbact,rabccbaaacbt 等等所在行
  ```
  ![1679487331812](image/Shell学习笔记/1679487331812.png)
- 特殊字符：**\\**
\是转义字符，若想匹配含有特殊字符的串，需用到转义字符，***注意需要使用单引号将表达式引起来***。
  ```
   cat /etc/passwd | grep ‘a\$b’     #匹配所有包含a$b 的行
  ```
## 9.文本处理工具
9.1 ***cut***
  9.1.1 基本用法
  cut [选项参数] filename
  从文件的每一行剪切字节、字符和字段并将这些字节、字符和字段输出；默认分隔符是制表符。
